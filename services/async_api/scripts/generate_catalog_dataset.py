...
CONFLICTS = [
    ...
    "пытается вернуть утраченную запись",
    "раскрывает чужую тайну",
    "готовится к решающему матчу",
    "спасает команду от распада",
    "разбирается в прошлом семьи",
    "идёт по следу забытого дела",
    "пытается остановить цепочку странных событий",
]
TWISTS = [
    "и постепенно меняет жизнь окружающих",
    "и находит неожиданных союзников",
    "и узнаёт, что ошибка была неслучайной",
    "и сталкивается с выбором между карьерой и близкими",
    "и открывает новый смысл своей работы",
    "и выходит на след старой легенды",
    "и понимает, что правда ближе, чем казалось",
    "и вынужден довериться человеку из прошлого",
]
PERSON_FIRST = [
    "Алексей",
    "Андрей",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Генерация и загрузка большого синтетического каталога в Elasticsearch."
    )
...
def synthetic_description(idx: int) -> str:
    ...
    twist = TWISTS[
        (idx // (len(HEROES) * len(SETTINGS) * len(CONFLICTS))) % len(TWISTS)
    ]
...
def person_name(role_prefix: str, idx: int) -> str:
    ...
    last_secondary = PERSON_LAST[
        (idx // (len(PERSON_FIRST) * len(PERSON_LAST))) % len(PERSON_LAST)
    ]
    if last_secondary == last_primary:
        last_secondary = PERSON_LAST[
            (PERSON_LAST.index(last_primary) + role_shift + 1) % len(PERSON_LAST)
        ]
...
def extra_genres() -> list[GenreRef]:
    ...
    for idx, (name, aliases) in enumerate(EXTRA_GENRES, start=1):
        refs.append(
            GenreRef(genre_id=make_uuid(GENRE_NS + idx), name=name, aliases=aliases)
        )
    return refs
...
def iter_person_movie_indexes(
    person_idx: int, pool: int, total_films: int, offsets: Iterable[int]
) -> Iterator[int]:
    seen: set[int] = set()
...
def index_documents(
    ...
    for doc in docs:
        ...
        lines.append(
            json.dumps(
                {"index": {"_index": index_name, "_id": doc_id}}, ensure_ascii=False
            )
            + "\n"
        )
        lines.append(json.dumps(doc, ensure_ascii=False) + "\n")
...
def iter_synthetic_person_docs(
    total_films: int, director_pool: int, actor_pool: int, writer_pool: int
) -> Iterator[dict]:
    for idx in range(director_pool):
        person = person_ref(PERSON_NS, "Р", "director", idx)
        yield person_doc(
            person, iter_person_movie_indexes(idx, director_pool, total_films, [0])
        )
    ...
    for idx in range(actor_pool):
        ...
        yield person_doc(
            person,
            iter_person_movie_indexes(idx, actor_pool, total_films, actor_offsets),
        )
    for idx in range(writer_pool):
        person = person_ref(PERSON_NS + 900_000_000, "С", "writer", idx)
        yield person_doc(
            person, iter_person_movie_indexes(idx, writer_pool, total_films, [0])
        )
...
def main() -> int:
    ...
    if not args.skip_recreate:
        ...
        recreate_index(
            args.elastic_url, INDEX_PERSONS, DATA_DIR / "persons.mapping.json"
        )

    base_genres: list[dict] = []
    if not args.without_base_fixtures:
        load_base_fixture_bulk(
            args.elastic_url, INDEX_FILMS, DATA_DIR / "movies.bulk.ndjson"
        )
        base_genres = load_base_fixture_bulk(
            args.elastic_url, INDEX_GENRES, DATA_DIR / "genres.bulk.ndjson"
        )
        load_base_fixture_bulk(
            args.elastic_url, INDEX_PERSONS, DATA_DIR / "persons.bulk.ndjson"
        )
    ...
    movie_count = index_documents(
        args.elastic_url,
        INDEX_FILMS,
        iter_synthetic_movie_docs(
            args.films, genre_pool, director_pool, actor_pool, writer_pool
        ),
