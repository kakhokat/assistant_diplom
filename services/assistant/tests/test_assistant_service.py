...
from services.session_store import InMemoryFeedbackStore
from services.session_store import InMemoryJsonCacheStore
from services.session_store import InMemoryLlmCircuitStore
...
class FakeCatalogClient:
    def __init__(self):
        ...
        self.genre_names = {
            'genre-sci-fi': {'uuid': 'genre-sci-fi', 'name': 'Фантастика', 'aliases': ['Sci-Fi']},
            'genre-drama': {'uuid': 'genre-drama', 'name': 'Драма', 'aliases': ['Drama']},
            'genre-thriller': {'uuid': 'genre-thriller', 'name': 'Детектив', 'aliases': ['Mystery']},
        }
        self.person_search_results = {
            'Луна Сильвер': [
                {'uuid': 'person-1', 'full_name': 'Луна Сильвер', 'aliases': ['Luna Silver']},
            ],
        }
    ...
    async def search_films(self, query: str, authorization: str) -> list[dict[str, Any]]:
        ...
        return [
            ...
            or any(query_lower in alias.lower() for alias in (film.get('title_aliases') or []))
        ]

    async def search_genres(self, query: str, authorization: str) -> list[dict[str, Any]]:
        query_lower = query.lower()
        return [
            {'id': genre_id, **payload}
            for genre_id, payload in self.genre_names.items()
            if query_lower in payload['name'].lower() or any(query_lower in alias.lower() for alias in payload.get('aliases', []))
        ]

    async def list_top_films(self, authorization: str, limit: int = 10) -> list[dict[str, Any]]:
    ...
    async def films_by_genre(self, genre_id: str, authorization: str) -> list[dict[str, Any]]:
        return [
            film for film in self.top_films if genre_id in self.film_details_map[film['uuid']].get('genre', [])
        ]

    async def search_persons(self, query: str, authorization: str) -> list[dict[str, Any]]:
        for key, value in self.person_search_results.items():
            if query.lower() in key.lower():
                return value
        return []

    async def person_details(self, person_id: str, authorization: str) -> dict[str, Any]:
...
class FakeUgcClient:
    ...
    async def bookmarks_by_user(self, user_id: str, authorization: str) -> list[dict[str, Any]]:
        return list(self.bookmarks)

    async def likes_by_user(self, user_id: str, authorization: str) -> list[dict[str, Any]]:
...
class FakeLlmClient:
    ...
    async def parse_query(self, query: str, session_context: dict[str, Any]) -> LlmParseResult | None:
        self.calls += 1
        return self.result
...
def build_service(
    ...
    *,
...
) -> tuple[AssistantService, FakeAuthClient, FakeCatalogClient, FakeUgcClient, FakeSessionStore]:
    ...
    service = AssistantService(
        ...
        llm_circuit_store=llm_circuit_store or InMemoryLlmCircuitStore(failure_threshold=1, cooldown_seconds=60),
    )
    return service, auth_client, catalog_client, ugc_client, shared_store


def test_recommend_general_returns_structured_recommendations_and_uses_service_token() -> None:
    service, auth_client, _, _, store = build_service()

    response = asyncio.run(service.handle_query('посоветуй фильм', authorization=None, session_id='sess-1'))
...
def test_bookmarks_without_auth_requires_login() -> None:
    ...
    response = asyncio.run(service.handle_query('что у меня в закладках?', authorization=None, session_id='sess-2'))
...
def test_film_followup_uses_shared_session_store_across_service_instances() -> None:
    ...
    asyncio.run(service_first.handle_query('посоветуй фильм', authorization=None, session_id='sess-4'))
    response = asyncio.run(service_second.handle_query('кто режиссёр этого фильма?', authorization=None, session_id='sess-4'))
...
def test_person_followup_uses_shared_session_store_across_service_instances() -> None:
    ...
    asyncio.run(
        service_first.handle_query('какие фильмы у "Луна Сильвер"?', authorization=None, session_id='sess-5')
    )
    response = asyncio.run(service_second.handle_query('сколько фильмов у неё?', authorization=None, session_id='sess-5'))
...
def test_detects_director_intent_for_kto_snyal() -> None:
    ...
    response = asyncio.run(service.handle_query('Кто снял фильм Сайлент мун?', authorization=None, session_id='sess-6'))
...
def test_director_answer_uses_russian_person_name_in_response() -> None:
    ...
    response = asyncio.run(
        service.handle_query('кто режиссёр фильма "Тихая Луна"?', authorization=None, session_id='sess-7')
    )
...
def test_detects_overview_intent_for_summary_phrases() -> None:
    ...
    response = asyncio.run(
        service.handle_query('дай короткую сводку по фильму Тихая Луна', authorization=None, session_id='sess-8')
    )
...
def test_llm_parse_result_is_cached_for_low_confidence_queries() -> None:
    ...
    first = asyncio.run(service.handle_query('объясни что там за кино тихая луна', None, 'sess-9'))
    second = asyncio.run(service.handle_query('объясни что там за кино тихая луна', None, 'sess-10'))
...
def test_public_film_response_is_cached_by_canonical_entity() -> None:
    ...
    first = asyncio.run(service.handle_query('кто режиссёр фильма Тихая Луна?', None, 'sess-11'))
...
def test_llm_circuit_breaker_opens_after_failure_and_skips_extra_llm_calls() -> None:
    ...
    service, _, _, _, _ = build_service(llm_client=llm_client, llm_circuit_store=circuit)

    first = asyncio.run(service.handle_query('что-то очень странное и непонятное', None, 'sess-13'))
    second = asyncio.run(service.handle_query('что-то очень странное и непонятное', None, 'sess-14'))
