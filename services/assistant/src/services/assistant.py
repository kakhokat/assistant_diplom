...
from services.entity_resolver import help_text, prepare_film_cards, remember
from services.query_handlers import (
    handle_bookmarks,
    handle_film_query,
    handle_person_filmography,
    handle_person_movie_count,
    handle_recommend_by_genre,
    handle_recommend_by_person,
    handle_recommend_general,
)
from services.query_parser import looks_like_film_followup, looks_like_person_followup
from services.query_plan import (
    QueryPlan,
    deterministic_plan,
    should_accept_deterministic_plan,
)
from services.session_store import (
    ...
    NullLlmCircuitStore,
    ParseCacheStore,
    PublicResponseCacheStore,
    RedisFeedbackStore,
    RedisLlmCircuitStore,
    RedisParseCacheStore,
    RedisPublicResponseCacheStore,
    RedisSessionStore,
    SessionStore,
)
...
class AssistantService:
    ...
    async def public_feed(
        self, limit: int, authorization: str | None
    ) -> list[dict[str, Any]]:
    ...
    async def submit_feedback(
        self, payload: AssistantFeedbackRequest
    ) -> AssistantFeedbackResponse:
        ...
        try:
            ...
            if plan.intent == "bookmarks":
                if not self._has_bearer(authorization):
                    response = self._auth_required_response(
                        query, current_session_id, session, "bookmarks"
                    )
                else:
                    response = await handle_bookmarks(
                        self, query, authorization, current_session_id, session
                    )
            elif plan.intent == "recommend_by_genre":
                if not self._has_bearer(authorization):
                    response = self._auth_required_response(
                        query, current_session_id, session, "recommend_by_genre"
                    )
                else:
                    response = await handle_recommend_by_genre(
                        self, query, authorization, current_session_id, session
                    )
            elif plan.intent == "recommend_general":
                response = await handle_recommend_general(
                    self, query, authorization, current_session_id, session
                )
            ...
            response.metadata["llm_fallback_used"] = plan.source in {
                "llm",
                "parse_cache",
            }
    ...
    async def _build_plan(
        self, query: str, session: dict[str, Any], parse_cache_key: str
    ) -> QueryPlan:
    ...
    async def _catalog_authorization(
    ...
    async def _catalog_call(
        self, method_name: str, *args: Any, authorization: str | None, **kwargs: Any
    ) -> Any:
        ...
        try:
            ...
            if exc.response is not None and exc.response.status_code == 401:
                refreshed_auth = await self.auth_client.service_authorization(
                    force_refresh=True
                )
                return await method(*args, refreshed_auth, **kwargs)
            raise

    async def _catalog_search_films(
        self, query: str, authorization: str | None
    ) -> list[dict[str, Any]]:
        return await self._catalog_call(
            "search_films", query, authorization=authorization
        )

    async def _catalog_search_genres(
        self, query: str, authorization: str | None
    ) -> list[dict[str, Any]]:
        return await self._catalog_call(
            "search_genres", query, authorization=authorization
        )

    async def _catalog_list_top_films(
        self, authorization: str | None, limit: int = 10
    ) -> list[dict[str, Any]]:
        return await self._catalog_call(
            "list_top_films", authorization=authorization, limit=limit
        )

    async def _catalog_film_details(
        self, film_id: str, authorization: str | None
    ) -> dict[str, Any]:
        return await self._catalog_call(
            "film_details", film_id, authorization=authorization
        )

    async def _catalog_genre_details(
        self, genre_id: str, authorization: str | None
    ) -> dict[str, Any]:
        return await self._catalog_call(
            "genre_details", genre_id, authorization=authorization
        )

    async def _catalog_films_by_genre(
        self, genre_id: str, authorization: str | None
    ) -> list[dict[str, Any]]:
        return await self._catalog_call(
            "films_by_genre", genre_id, authorization=authorization
        )

    async def _catalog_search_persons(
        self, query: str, authorization: str | None
    ) -> list[dict[str, Any]]:
        return await self._catalog_call(
            "search_persons", query, authorization=authorization
        )

    async def _catalog_person_details(
        self, person_id: str, authorization: str | None
    ) -> dict[str, Any]:
        return await self._catalog_call(
            "person_details", person_id, authorization=authorization
        )
    ...
    @staticmethod
    def _public_response_cache_key(
        intent: str, *, entity_type: str, entity_id: str
    ) -> str:
    ...
    async def _maybe_store_public_response(
        self, response: AssistantResponse, authorization: str | None
    ) -> None:
        if self._has_bearer(authorization):
            return
...
def build_assistant_service(
    client: httpx.AsyncClient, session_redis: Any
) -> AssistantService:
