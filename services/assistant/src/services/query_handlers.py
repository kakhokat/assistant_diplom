...
from services.entity_resolver import display_title
from services.entity_resolver import format_person_names_for_answer
from services.entity_resolver import genre_display_name
from services.entity_resolver import load_film_details
from services.entity_resolver import pick_single_recommendation
from services.entity_resolver import remember
from services.entity_resolver import resolve_explicit_genre
from services.entity_resolver import resolve_film
from services.entity_resolver import resolve_genre_names
from services.entity_resolver import resolve_person
from services.query_parser import candidate_queries
...
async def handle_film_query(
    ...
    film, alternatives, used_query = await resolve_film(service, title, search_queries or [], authorization)
    if not film:
        return service._response(
            query=query,
            session_id=session_id,
            ...
            metadata={'film_query': title, 'search_queries': candidate_queries(title, search_queries or [])},
            context=session,
            alternatives=alternatives,
        )

    cache_key = service._public_response_cache_key(intent, entity_type='film', entity_id=str(film['uuid']))
    if not service._has_bearer(authorization):
        cached = await service._load_cached_public_response(cache_key, query, session_id, session)
        if cached is not None:
            return cached

    detail = await service._catalog_film_details(film['uuid'], authorization)
    genre_names = await resolve_genre_names(service, detail.get('genre') or [], authorization)
    matched_title = display_title(detail or film) or detail.get('title', film.get('title', title))
    ...
    if intent == 'film_rating':
        ...
        formatted_directors = await format_person_names_for_answer(service, directors, authorization) if directors else []
...
async def handle_person_movie_count(
    ...
    person, alternatives, used_query = await resolve_person(service, raw_name, search_queries or [], authorization)
    if not person:
        return service._response(
            query=query,
            session_id=session_id,
            ...
            confidence=0.2,
            used_services=['catalog'],
            metadata={'person_query': raw_name, 'search_queries': candidate_queries(raw_name, search_queries or [])},
            context=session,
            alternatives=alternatives,
        )
    cache_key = service._public_response_cache_key('person_movie_count', entity_type='person', entity_id=str(person['uuid']))
    if not service._has_bearer(authorization):
        cached = await service._load_cached_public_response(cache_key, query, session_id, session)
        if cached is not None:
            return cached
    ...
    remember(session, {'person_id': detail.get('uuid', person.get('uuid')), 'person_name': matched_name})
...
async def handle_person_filmography(
    ...
    cache_key = service._public_response_cache_key('person_filmography', entity_type='person', entity_id=str(person['uuid']))
    if not service._has_bearer(authorization):
        cached = await service._load_cached_public_response(cache_key, query, session_id, session)
        if cached is not None:
            return cached
    ...
    if not films:
        ...
        film_details = await load_film_details(service, [item.get('uuid') for item in films if item.get('uuid')], authorization)
...
async def handle_bookmarks(service: Any, query: str, authorization: str, session_id: str, session: dict[str, Any]):
    ...
    details = await load_film_details(service, [item['film_id'] for item in bookmarks], authorization)
    titles = ', '.join(display_title(detail) for detail in details[:5] if display_title(detail))
...
async def handle_recommend_by_genre(service: Any, query: str, authorization: str, session_id: str, session: dict[str, Any]):
    me = await service._me_or_none(authorization)
    if me is None:
        return service._auth_required_response(query, session_id, session, 'recommend_by_genre')
    bookmarks = await service.ugc_client.bookmarks_by_user(str(me['id']), authorization)
    ...
    items = ', '.join(f"{display_title(item)} ({item.get('imdb_rating', 'n/a')})" for item in recommended)
...
async def handle_recommend_general(
    ...
    if explicit_genre is not None:
        return await handle_public_recommend_by_genre(service, query, authorization, session_id, session, explicit_genre)

    top_films = await service._catalog_list_top_films(authorization, limit=max(settings.RECOMMENDATION_LIMIT, 10))
    if not top_films:
        return service._response(
            query=query,
            session_id=session_id,
    ...
    choice = pick_single_recommendation(session, top_candidates, 'general_recommendations')
    display = display_title(choice)
    remember(session, {'film_id': choice.get('uuid'), 'film_title': display})
    return service._response(
        query=query,
...
async def handle_recommend_by_person(
    ...
    if not films:
        ...
        answer = f'Могу предложить фильмы с участием или от {matched_name}: ' + ', '.join(
            f"{display_title(item)} ({item.get('imdb_rating', 'n/a')})" for item in items
        ) + '.'
    return service._response(
        query=query,
        session_id=session_id,
