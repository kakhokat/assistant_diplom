...
class RedisSessionStore:
    def __init__(self, redis: Any, ttl_seconds: int, key_prefix: str = 'assistant_session') -> None:
        self.redis = redis
    ...
    async def save(self, session_id: str, session: dict[str, Any]) -> None:
        ...
        await self.redis.set(self._key(session_id), orjson.dumps(payload), ex=self.ttl_seconds)
...
class RedisJsonCacheStore:
    ...
    async def save(self, cache_key: str, payload: dict[str, Any]) -> None:
        await self.redis.set(self._key(cache_key), orjson.dumps(payload), ex=self.ttl_seconds)
...
class RedisLlmCircuitStore:
    def __init__(self, redis: Any, key_prefix: str, failure_threshold: int, cooldown_seconds: int) -> None:
        self.redis = redis
        self.key_prefix = key_prefix
    ...
    async def _load_state(self) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            return {'failures': 0, 'open_until': 0}
        return {'failures': int(payload.get('failures', 0)), 'open_until': int(payload.get('open_until', 0))}
