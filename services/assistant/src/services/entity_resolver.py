...
from services.query_parser import extract_explicit_genre_hint
...
async def prepare_film_cards(
    ...
    for item in items:
        ...
        genres = await resolve_genre_names(service, detail.get('genre') or [], authorization) if detail else []
        prepared.append(
            {
                'uuid': item.get('uuid'),
...
async def format_person_name_for_answer(service: Any, name: str, authorization: str | None) -> str:
    try:
...
async def format_person_names_for_answer(service: Any, names: list[str], authorization: str | None) -> list[str]:
    return [await format_person_name_for_answer(service, name, authorization) for name in names]
...
async def resolve_explicit_genre(service: Any, query: str, authorization: str | None) -> dict[str, Any] | None:
    ...
    best = pick_best_candidate(
        hint,
        genres,
        lambda item: ' '.join([item.get('name', ''), ' '.join(item.get('aliases') or [])]),
    ) or genres[0]
    return best



def pick_single_recommendation(session: dict[str, Any], items: list[dict[str, Any]], key: str) -> dict[str, Any]:
...
async def load_film_details(service: Any, film_ids: Iterable[str], authorization: str | None) -> list[dict[str, Any]]:
...
async def resolve_genre_names(service: Any, genre_ids: Iterable[str], authorization: str | None) -> list[str]:
    names: list[str] = []
